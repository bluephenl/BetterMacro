...
function ActionInspector({ action, index, onChange, onDelete, onDuplicate, onMove, onRun }: { action: Action; index: number; onChange: (index: number, action: Action) => void; onDelete: () => void; onDuplicate: () => void; onMove: (index: number, direction: -1 | 1) => void; onRun: () => void }) {
  ...
  return <div className="inspector-body">
    ...
    {(action.kind === "click" || action.kind === "scroll" || action.kind === "move" || action.kind === "drag") && <div className="inspector-section"><span>Targeting</span>{(action.kind === "click" || action.kind === "scroll") && action.target ? <><div className="target-option selected"><i/> Semantic UI element</div><span className="field-note">{semanticTargetLabel(action.target)}</span><div className="target-option"><i/> Screen coordinates <em>used only for macros without semantic data</em></div></> : <div className="target-option selected"><i/> Screen coordinates</div>}<div className="target-option muted"><i/> Image target <em>not yet available</em></div></div>}
